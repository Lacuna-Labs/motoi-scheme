...
**→ For our DSL:** Motoi Scheme already gives us homoiconic `(verb args)`. Win comes from (a) writing an **EBNF + running XGrammar-style constrained decode** on the 4B, (b) a **closed, ~25-verb vocabulary with fixed arity and enum constants**, (c) banning arity-overloading. This is the single highest-leverage item in the brief.
...
| Motion authored in Motoi Scheme via the REAL interpreter | BUILT (invariant honored) | the JS harness is a headless surface + encoder ONLY; motion is Scheme through the interpreter (the `render_grid_anim.mjs` JS-waltz anti-pattern is not reintroduced) |
