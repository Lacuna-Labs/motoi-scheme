...
A small Scheme dialect, written in JavaScript, that runs anywhere Node runs. The engine is under two thousand lines. The language is R7RS-shaped where R7RS is sensible, and a few careful modules where it isn't. It's meant to be programmed with — by you, by a friend, by an AI collaborator sitting in the same REPL as you.
...
- **Fragmented ecosystem.** — There is one Motoi Scheme. R7RS subset for the core; modules marked as ours.
